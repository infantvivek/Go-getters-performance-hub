...
def load_and_standardize(url, sheet_type):
    try:
        ...
        if sheet_type == "KPI":
            ...
            # Normalizing specific month typos before datetime parsing
            if 'date_raw' in df.columns:
                df['date_raw'] = df['date_raw'].astype(str).str.replace('Sept', 'Sep', case=False)
...
with tab_dsat:
    ...
    with tab_pos:
        ...
        if not pos_df.empty:
            ...
            elif pos_filter == "Without Comments":
                no_comments = pos_table['customercomments'].isna() | pos_table['customercomments'].astype(str).str.strip().str.lower().isin(['', 'nan', '-', 'none', 'null'])
                pos_table = pos_table[no_comments]
